...
export default function LanguageSelectRow(props: { position: number, hasError?: boolean, required?: boolean, onDelete?: (position: number) => void, language: string, setLanguage: (position: number, language: string) => void }) {
    return (
        <ListItem>
            <ListItemText primary={
                <Fragment>
                    <Typography variant="h5" display='inline' component='span' color={props.hasError ? 'error' : 'white'}><Translator path='language_selector.title' /></Typography>
            ...
            <Stack direction='row' spacing={2}>
                <span />
                <LanguageSelect language={props.language} setLanguage={value => props.setLanguage(props.position, value)} />
...
function LanguageSelect(props: { language: string, setLanguage: (language: string) => void }) {
